...
signed main() {
    ...
    if (p==1) {
        ...
        if ((32-__builtin_clz(max1))*counts.size() < (32-__builtin_clz(max2))*counts2.size()) {
            ...
            while(i<counts.size()){
                cerr << i << '\n';
                while(i<counts.size() &&32-__builtin_clz(*max_element(counts.begin()+i,counts.end())) == b) {
                    tmp.push_back(counts[i]);
                    ++i;
                }
                cout << int_to_bits(tmp.size(),5);
                for (int t:tmp) {
                    cout << int_to_bits(t,b);
                }
                b--;
                tmp.clear();
            }
            ...
            vector<int> tmp;
            cout << int_to_bits(b,4);
            while(i<counts2.size()){
                cerr << b << ": ";
                while(i<counts2.size() && 32-__builtin_clz(*max_element(counts2.begin()+i,counts2.end())) == b) {
                    tmp.push_back(counts2[i]);
                    ++i;
                }
                cout << int_to_bits(tmp.size(),5);
                cerr << tmp.size() << '\n';
