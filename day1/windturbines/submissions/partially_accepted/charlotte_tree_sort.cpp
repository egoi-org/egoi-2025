...
signed main(){
    ...
    range.insert({preorder[0],0});
    vector<int> is_del(2*n,0);
    for(int j=1; j<=q;++j){
        //fprintf(stderr,"processing query %d\n",queries[j].first);
        for(int i=queries[j-1].first+1;i<=queries[j].first;++i){
            //fprintf(stderr,"start\n");
            //fprintf(stderr,"processing vertex %d\n",i);
            auto p = range.upper_bound({preorder[i],i});
            if (p!= range.begin()) {
                //fprintf(stderr,"test %d\n", (*prev(p)).second);
                int next = (*prev(p)).second;
                int lca = find_max(next,i);
                //fprintf(stderr,"lca of %d and %d is %d cost %d\n",next,i,lca,edge_costs[lca]);
                ...
                if (!is_del[lca]) {
                    ...
                    assert(p != range.end());
                    int lca = find_max((*p).second,i);
                     del += edge_costs[lca]; 
                    is_del[lca]=true;
                    //fprintf(stderr,"lca of %d and %d is %d cost %d\n",(*p).second,i,lca,edge_costs[lca]);
                }
            } else {
                    int lca = find_max((*p).second,i);
                     del += edge_costs[lca]; 
            ...
            range.insert({preorder[i],i});
        }
        //fprintf(stderr,"done\n");
        ans[queries[j].second] = cost - del;
    }
    for (ll a : ans) {
        cout << a << '\n';
    }
